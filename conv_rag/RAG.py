...
STUDENT_CSV_PATH = ""  # Empty to force Django data usage
...
def generate_with_openai(prompt, model_name=None, temperature=0.7, max_tokens=1000):
    try:
        llm = create_azure_chat_openai(temperature=temperature, max_tokens=max_tokens)
        ...
        return f"❌ Error with OpenAI API: {str(e)}", 0, 0
...
def generate_single_question(essay_topic, college_name, major, word_limit, question_number, conversation_history, top_k=3):
    ...
    question, input_tokens, output_tokens = generate_with_openai(prompt, max_tokens=100)
...
def generate_follow_up_question(essay_topic, student_name, last_question, last_answer, student_profile):
    ...
    try:
        follow_up, input_tokens, output_tokens = generate_with_openai(prompt, max_tokens=100)
